...
import { PrinterIcon, CreditCardIcon, ArrowTopRightOnSquareIcon } from '@heroicons/react/24/outline';
import { useSortableData } from '@/lib/sort';
import SortableHeader from '@/Components/SortableHeader';

export default function LaporanHutang({ hutang, total_belum_lunas, filters }) {
    const { items: sortedHutang, sortConfig, requestSort } = useSortableData(hutang.data);
    const [status, setStatus] = useState(filters.status ?? 'belum_lunas');

    const handleFilter = (st) => {
        setStatus(st);
        router.get(route('laporan.hutang'), { status: st }, { preserveState: true });
    };

    const handlePrint = () => window.print();

    return (
        <AppLayout title="Laporan Hutang Supplier">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="print:hidden">
                    <LaporanTabs activeTab="laporan.hutang" />
                </div>

                {/* Print Header */}
                <div className="hidden print:block mb-6 border-b pb-4">
                    <h1 className="text-2xl font-bold text-slate-900">Laporan Hutang ke Supplier</h1>
                    <p className="text-sm text-slate-600">Dicetak pada {new Date().toLocaleDateString('id-ID')}</p>
                </div>

                {/* Card Summary */}
                <div className="bg-gradient-to-r from-red-500/20 via-purple-500/10 to-transparent print:bg-slate-100 border border-red-500/30 print:border-slate-300 rounded-2xl p-6 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-red-500/20 border border-red-500/40 flex items-center justify-center text-red-400 print:text-slate-900 shrink-0">
                            <CreditCardIcon className="w-6 h-6" />
                        </div>
                        <div>
                            <p className="text-xs font-bold text-red-300 print:text-slate-600 uppercase tracking-wider">Total Hutang Belum Lunas (Kewajiban Bayar)</p>
                    ...
                    <div className="flex gap-2 print:hidden">
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                                status === 'belum_lunas' ? 'bg-red-500 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'
                            }`}
                        >
                            Belum Lunas
                        </button>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-slate-700/50 print:border-slate-400 text-xs text-slate-400 print:text-slate-600 uppercase tracking-wider">
                                    <SortableHeader label="Tanggal / Faktur" sortKey="tanggal_hutang" currentSort={sortConfig} onSort={requestSort} align="left" />
                                    <SortableHeader label="Nama Supplier" sortKey="supplier.nama" currentSort={sortConfig} onSort={requestSort} align="left" />
                                    <SortableHeader label="Total Hutang" sortKey="jumlah_hutang" currentSort={sortConfig} onSort={requestSort} align="right" />
                                    <SortableHeader label="Sudah Dibayar" sortKey="jumlah_terbayar" currentSort={sortConfig} onSort={requestSort} align="right" />
                                    <SortableHeader label="Sisa Kewajiban" sortKey="sisa_hutang" currentSort={sortConfig} onSort={requestSort} align="right" />
                                ...
                                {sortedHutang.length === 0 ? (
                                    <tr>
                                        <td colSpan={7} className="px-5 py-12 text-center text-slate-500">
                                            Tidak ada data hutang pada filter ini.
                                        </td>
                                    </tr>
                                ) : (
                                    sortedHutang.map((h) => (
                                        <tr key={h.id} className="hover:bg-slate-800/40 print:text-slate-900 transition-colors">
                                            <td className="px-5 py-3.5">
                                                <p className="font-semibold text-white print:text-slate-900">{tanggalIndo(h.tanggal_hutang)}</p>
                                                <p className="text-xs text-slate-500">{h.pembelian?.nomor_faktur || `PB-${h.pembelian_id}`}</p>
                                            </td>
                                            <td className="px-5 py-3.5">
                                                <p className="font-bold text-slate-200 print:text-slate-900">{h.supplier?.nama || 'Tanpa Supplier'}</p>
                                            </td>
                                            ...
                                            <td className="px-5 py-3.5 text-center">
                                                <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase ${
                                                    h.status === 'lunas' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'
                                                }`}>
                                                    {h.status?.replace('_', ' ')}
                                                </span>
                                            </td>
